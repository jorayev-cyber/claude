...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['attendance'])) {
    foreach ($_POST['attendance'] as $studentId => $status) {
        ...
        if ($existing) {
            $db->update('attendance', ['status' => $status, 'marked_by' => $_SESSION['user_id']], 'id = ?', [$existing['id']]);
        } else {
...
$groups = $db->fetchAll("SELECT g.*, c.name as course_name FROM groups_table g JOIN courses c ON g.course_id = c.id WHERE g.status = 'active' ORDER BY g.name");
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php foreach ($students as $i => $s): 
                            $currentStatus = $attendanceMap[$s['id']] ?? 'present';
                        ?>
                        <tr>
                            <td><?= $i+1 ?></td>
                            <td><strong><?= clean($s['full_name']) ?></strong></td>
                            <td><input type="radio" name="attendance[<?= $s['id'] ?>]" value="present" <?= $currentStatus=='present'?'checked':'' ?> class="form-check-input"></td>
                            <td><input type="radio" name="attendance[<?= $s['id'] ?>]" value="absent" <?= $currentStatus=='absent'?'checked':'' ?> class="form-check-input"></td>
                            <td><input type="radio" name="attendance[<?= $s['id'] ?>]" value="late" <?= $currentStatus=='late'?'checked':'' ?> class="form-check-input"></td>
                            <td><input type="radio" name="attendance[<?= $s['id'] ?>]" value="excused" <?= $currentStatus=='excused'?'checked':'' ?> class="form-check-input"></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
<?php elseif ($groupId): ?>
<div class="alert alert-info">Bu guruhda o'quvchilar yo'q</div>
<?php endif; ?>
